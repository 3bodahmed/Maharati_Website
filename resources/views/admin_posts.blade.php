...
<head>
    ...
    <title>إدارة المنشورات - مهارتي</title>
    ...
    <style>
        ...
        .post-type-badge.client { background: #dbeafe; color: #1a73e8; }
        .post-type-badge.provider { background: #d1fae5; color: #059669; }
    ...
    <main class="main-content">
        ...
        <section class="hero-section" style="padding: 30px 20px;">
            <div class="hero-glass-card" style="padding: 25px 30px;">
                <h1 style="font-size: 2rem;">📄 إدارة <span>المنشورات</span></h1>
                <p style="opacity:0.7;">عرض وحذف المنشورات المنشورة على المنصة</p>
            </div>
        </section>

        <div class="admin-section-header">
            <h2 style="color: var(--gold);"><i class="fas fa-file-alt"></i> قائمة المنشورات</h2>
            <div style="display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
                <span style="opacity:0.6;">إجمالي: {{ $posts->total() }} منشور</span>
                {{-- ===== صندوق البحث ===== --}}
                <form method="GET" action="{{ route('admin.posts') }}" class="search-box-admin">
                    <input type="text" name="search" placeholder="بحث بالعنوان، المحتوى، أو الموقع..." value="{{ request('search') }}"  style="color: #000" autocomplete="off" />
                    <button type="submit"><i class="fas fa-search"></i></button>
                    @if(request('search'))
                        <a href="{{ route('admin.posts') }}" class="clear-btn visible" style="color:#666; text-decoration:none; padding:8px 12px;">
                ...
                <tbody>
                    @forelse($posts as $post)
                        <tr>
                            ...
                            <td style="max-width:180px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">{{ $post->content }}</td>
                            <td>
                                <span class="post-type-badge {{ $post->typeRequest }}">
                                    {{ $post->typeRequest == 'client' ? 'طلب خدمة' : 'خدمة مقدمة' }}
                                </span>
                            </td>
                            <td>{{ $post->price }} ريال</td>
                            <td>{{ $post->location }}</td>
                            ...
                            <td>
                                <form action="{{ route('admin.posts.delete', $post->id) }}" method="POST" style="display:inline-block;">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="action-btn delete" onclick="return confirm('حذف المنشور؟')">
